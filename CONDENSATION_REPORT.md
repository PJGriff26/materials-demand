...
*Historical note (2026-08-07): this report predates the manuscript figure
renumbering and is preserved unchanged; the figure numbers and script names in
the body use the old scheme. Mapping to the current numbering: old Figure 4
(sensitivity tornado) is now Figure S3 (`fig4_sensitivity_tornado.py` was
renamed `figS3_sensitivity_tornado.py`); the old combined 4-panel supply figure
"Figure 5" was split into the current Figures 4 (US supply tiers) and 5 (global
supply tiers), generated by `fig4_fig5_supply_tiers.py`; old Figure 7 (NIR x
HHI supply-risk scatter) is now Figure 6 (`fig6_nir_vs_hhi_scatter.py`); the
demand-by-technology figure is now Figure S1. Helper renames:
`fig4_supply_chain.py` is now `supply_tiers_shared.py` and
`fig5_alt_variants.py` is now `supply_tiers_variants.py`. The "seven figures"
count refers to the figure set at condensation time; the current set is nine
(Figures 1-6 and S1-S3).*
